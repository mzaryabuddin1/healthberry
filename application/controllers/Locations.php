...
class Locations extends CI_Controller
{
    ...
    public function edit($id)
    {
        ...
        $data['cities'] = $this->Products_model->get_active_cities();
        $data['data'] = $this->Locations_model->get_locations();
        $data['saved_data'] = $this->Locations_model->get_locations_via_id($id);
        $this->load->view('edit_locations_view', $data);
    }
    ...
    public function edit_location_submit($id)
    {
        // Form validation rules
        $this->form_validation->set_rules('doctor_name', 'Doctor Name', 'trim|required');
        $this->form_validation->set_rules('products[]', 'Products', 'trim|required');
        ...
        if ($this->form_validation->run() === FALSE) {
            ...
            foreach ($this->input->post('days') as $day) {
                $obj = new stdClass;
                $obj->dayname = $day;
                $obj->from = $this->input->post('timings_from')[$i];
                $obj->to = $this->input->post('timings_to')[$i];
                $timings_arr[] = $obj;
                $i++;
            }
            ...
            $data = [
                ...
                'latitude' => $this->input->post('latitude'),
                'longitude' => $this->input->post('longitude'),
                'timings' => json_encode($timings_arr),
                'updated_at' => date('Y-m-d H:i:s'),
                'updated_by' => $_SESSION['user_id']
            ];

            // Update the database
            $updated = $this->Locations_model->update_location($id, $data);
